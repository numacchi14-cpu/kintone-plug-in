...
- 作業開始前に`git push`でoriginへ退避する。ローカルブランチが未プッシュ・未マージのまま長期間残ることがあるため（2026-08-12のClaude Code精査で`agent/documentation-status`が9コミット未プッシュ・`main`未マージだったことを確認、詳細は`SPEC.md`の「Claude Codeによる精査（2026-08-12）」参照）。
