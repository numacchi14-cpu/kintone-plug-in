...
- 現行リリース: **v0.16.5**（コード・パッケージ作成済み。実Kintoneでの再出力検証は未実施）
...
- 単月（2025年11月）は旧帳票との1,518セル全件比較が差異0件。ただしこれはv0.16.3で日付正規化を導入する前の出力での確認であり、v0.16.5適用後の再検証がまだ済んでいない（上記「次の作業」参照）。
...
1. **v0.16.5の実Kintone検証（未実施）。** Claude Codeはこの案件用のKintone接続情報を持たないため、コード修正・テンプレート修正・パッケージ作成までは完了しているが、実出力での確認ができていない。Kintone接続情報を持つ環境で、2025年11月単月を再出力して金額が表示されること、同じ条件の累計出力（テンプレート・数式は無変更）で金額が変わらないことを確認してから本番へ適用する。
2. 上記確認後、累計の旧帳票との沖縄・九州全セル比較を行う。
3. `agent/documentation-status`ブランチの最新コミットをoriginへプッシュし、`main`へのマージも検討する（v0.16.5のコミットがまだ残っている場合）。
