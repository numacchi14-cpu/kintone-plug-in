...
- 現行リリース: **v0.16.5**（コード・パッケージ作成済み。単月は実Kintoneでの再出力検証も完了。累計は実出力の生データ突合で「1月分の欠落が解消された」ことまで確認済みだが、旧帳票との全セル照合はまだ）
- 配布ZIP: `dist/kintone-excel-report-plugin-v0.16.5.zip`
...
- **重要**：v0.16.4以前の累計出力は、期間開始月（1月）のレコードだけが`SUMIFS`の範囲条件から静かに除外される不具合を抱えていた（`SPEC.md`「単月の日付不具合とv0.16.5での修正」参照）。「累計は実Kintone出力で金額・書式を目視確認済み」という記述は、この欠落に気づく前の確認である。v0.16.5でこの欠落は解消され、修正前後の実出力を突合して1月分の金額が正しく戻ったことを確認済みだが、**旧累計帳票との沖縄・九州の全セル照合はまだ実施していない**。
...
1. 累計の旧帳票との沖縄・九州全セル比較を行う（単月は実出力データでの独立検証まで完了済み）。v0.16.5で期間開始月の欠落は解消されているはずだが、目視確認だけでは同種の集計範囲の誤りを見逃す実例が今回あったため、必ず値・数式結果を機械的に全セル照合すること。
2. `agent/documentation-status`ブランチの最新コミットをoriginへプッシュし、`main`へのマージも検討する（v0.16.5のコミットがまだ残っている場合）。
